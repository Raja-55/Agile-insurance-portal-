...
const SupportPage = () => {
  ...
  // API returns flat shape: { id, userName, userEmail, userPhone, ... }
  // so use chat.userName directly — NOT chat.user?.name
  const selectedChat = chats.find((c) => c.id === selectedChatId) || null;

  useEffect(() => {
    const load = async () => {
      const res = await apiRequest("/api/admin/support-tickets", { useAdminToken: true });
      if (res?.data) dispatch(setChats(res.data));
    };
    load();
  }, []);
  ...
  const handleReply = async () => {
    ...
    try {
      ...
      if (res?.data) {
        ...
        log(`replied -> ${selectedChat.userName}`);
        panel("Reply sent", `Admin response sent to ${selectedChat.userName}.`);
      }
    } catch (err) {
      panel("Error", `Failed to send reply: ${err.message}`);
    }
    setAdminReply("");
  };
  ...
  const resolveChat = async () => {
    ...
    log(`resolved -> ${selectedChat.userName}`);
    dispatch(setSelectedChatId(null));
    panel("Ticket resolved", `Ticket for ${selectedChat.userName} marked as resolved.`);
  };
        ...
        <div className="max-h-[600px] space-y-2 overflow-y-auto rounded-lg border border-slate-200 bg-slate-50 p-4">
          {chats.length === 0 ? (
            ...
            chats.map((chat) => (
              ...
              >
                {/* userName comes directly from API flat shape */}
                ...
                {(selectedChat.messages || []).map((msg) => (
                  <div key={msg._id || msg.id}>
                    <div className="text-xs font-bold uppercase text-slate-500">
                      {msg.senderRole === "admin" ? "Admin" : selectedChat.userName}
                    </div>
                    <div
              ...
              {selectedChat.status !== "Resolved" && (
                <div className="border-t border-slate-200 p-4 space-y-3">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={handleReply}
                      className="flex-1 inline-flex items-center justify-center gap-2 rounded-lg bg-blue-600 px-4 py-3 text-sm font-black text-white hover:bg-blue-700"
                    >
