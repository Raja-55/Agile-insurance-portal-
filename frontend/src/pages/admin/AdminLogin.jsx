...
const AdminLogin = () => {
  ...
  const onSubmitForgotPassword = async (e) => {
    ...
    setError("");
    setForgotNotice("");
    const emailToReset = forgotEmail.trim() || loginEmail.trim();
    if (!emailToReset) {
    ...
    setSubmitting(true);
    try {
      const res = await apiRequest("/api/admin/auth/forgot-password", {
        method: "POST",
        body: JSON.stringify({ email: emailToReset }),
      });
      if (res?.success) {
        setForgotNotice("A reset link has been sent to your admin email address.");
      } else {
  ...
  const onSubmitLogin = async (e) => {
    ...
    try {
      if (otpEnabled) {
        const res = await apiRequest("/api/admin/auth/verify-2fa", {
          method: "POST",
          body: JSON.stringify({
          ...
          {forgotView ? (
            <form onSubmit={onSubmitForgotPassword} className="space-y-4">
              <div className="flex items-center gap-2 text-blue-400">
                <ShieldCheck size={20} />
                <h2 className="text-base font-black text-white">Reset Admin Password</h2>
              </div>
              ...
              <label className="block">
                ...
                <div className="relative mt-2">
                  ...
                  <input
                    ...
                    className="h-12 w-full rounded-xl border border-white/10 bg-white/5 pl-10 pr-4 text-sm font-bold text-white outline-none focus:border-blue-500"
                  />
                </div>
              </label>

              {forgotNotice && (
                <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/10 px-3 py-2.5 text-xs font-bold text-emerald-300">
                  {forgotNotice}
                </div>
              )}
              {error && (
                <div className="rounded-xl border border-rose-500/20 bg-rose-500/10 px-3 py-2.5 text-xs font-bold text-rose-300">
                  {error}
                </div>
              )}
              ...
              <div className="flex flex-col gap-2 pt-2">
                <button
                  disabled={submitting}
                  className="inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-500 text-sm font-black text-white shadow-lg transition hover:opacity-90 disabled:opacity-60"
                >
                ...
                <button
                  ...
                  className="text-xs font-black text-slate-400 hover:text-white py-1 transition"
                >
                  Back to Login
                </button>
              </div>
            ...
            <form onSubmit={onSubmitLogin} className="space-y-4">
                      ...
                      <div className="max-h-64 overflow-y-auto p-1.5">
                        ...
                        {filteredAdmins.map((a) => (
                          <button
                            ...
                            className={`flex w-full items-center gap-3 rounded-lg px-2.5 py-2 text-left transition ${selectedId === a._id ? "bg-blue-500/15" : "hover:bg-white/5"
                              }`}
                          >
                            <span
                              className={`grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-gradient-to-br text-xs font-black text-white ${roleGradient(
              ...
              <div className="rounded-xl border border-white/10 bg-white/5">
                ...
                >
                  ...
                  <span
                    className={`text-xs font-black ${otpEnabled ? "text-emerald-400" : "text-blue-400"
                      }`}
                  >
